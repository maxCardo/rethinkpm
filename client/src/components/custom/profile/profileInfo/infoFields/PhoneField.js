...
const PhoneField = ({ updatePhone, tglAddPhoneMod, data }) => {
  ...
  let phone =
    ...
    data.phoneNumbers.find((phone) => phone.isPrimary)
      ? data.phoneNumbers.find((phone) => phone.isPrimary).number
      : "no phone on file";
    ...
    <Fragment key="phoneNumber">
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Are you sure you want to change the primary phone number
          </Modal.Title>
        </Modal.Header>
        <Modal.Footer className="modalFooterBtns">
          <Button
            ...
            variant="secondary"
            onClick={() => editPrimePhone(profileType.current)}
          >
            Confirm
          </Button>
          <Button
