...
export default function(state = initialState, action) {
    ...
    switch (type) {
        ...
        case OPEN_INQUIRY_CHAT:
            ...
            if (chat) {
                const isAlreadyOpen = openedChatsRightNow.find(
                    (openChatId) => openChatId === chat.id
                );
                if (isAlreadyOpen) {
                    return {
                        ...state,
                ...
                fetch(`/api/rent_lead/chat/${inquiryId}`, {
                    ...
                    .then((json) =>
                        dispatch({type: ADD_AND_OPEN_CHAT, payload: json})
                    );
                return state;
            }
        case ADD_AND_OPEN_CHAT:
            ...
            const newChat = {
                ...
                messages: payload.messages.map((message) => ({
                    ...
                    sender:
                        message.from == 'User-SMS'
                            ? payload.inq.prospect.name
                            : message.from,
