...
const mlsSold = async (html) => {
    ...
    /*ID*/
    const mlsIds = $("#x_wrapperTable table table tbody > tr:nth-child(3) a").text();

    /*PRICES*/
    let mlsPrices = $("#x_wrapperTable table table tbody > tr:nth-child(3) > td:last-child span").text().split("$");
    mlsPrices.shift();

    /*LOCATION DATA*/
    let mlsLoc =   $('#x_wrapperTable table tbody > tr:nth-child(2) > td:nth-child(2) tbody > tr:nth-child(2) span').map(function(){
        return $(this).text().replace(/ +(?= )/g,'');
    }).get();

    /*PROPERTY TYPE*/
    let mlsTypes = $("#x_wrapperTable table table tbody > tr:nth-child(4) > td:last-child span").text();

    /*PROPERTY # of BEDROOMS*/
    let mlsBedrooms = $("#x_wrapperTable table table tbody > tr:nth-child(5) > td:nth-child(2) span").text();

    /*LISTING STYLE*/
    let mlsStyle = $("#x_wrapperTable table table tbody > tr:nth-child(6) > td:nth-child(2) span").text();

    /*PARKING SPACE*/
    let mlsParking = $("#x_wrapperTable table table tbody > tr:nth-child(7) > td:nth-child(2)").map(function(){
    ...
    /*LISTING STYLE*/
    let mlsAcres = $("#x_wrapperTable table table tbody > tr:nth-child(8) > td:nth-child(2) span").text();

    /*BUILDING ENTRY OBJECT*/
    let locationArray = mlsLoc[0].split(",");
    let entryAddress =  locationArray[0];
    let entryZip = '';
...
const  mlsPriceDec = async (html) => {
    ...
    /*LISTING STYLE*/
    let mlsAcres = $("#x_x_wrapperTable table table tbody > tr:nth-child(8) > td:nth-child(2) span").map(function(){
        return $(this).text();
    }).get();
...
const mlsNew = async (html) => {
    ...
    const $ = cheerio.load(data)
    const setLength = $("#x_wrapperTable").length;
    ...
    /*PROPERTY TYPE*/
    let mlsTypes = $("#x_wrapperTable table table tbody > tr:nth-child(4) > td:last-child span").map(function(){
        return $(this).text();
    }).get();

    /*PROPERTY # of BEDROOMS*/
    let mlsBedrooms = $("#x_wrapperTable table table tbody > tr:nth-child(5) > td:nth-child(2) span").map(function(){
        return $(this).text();
    }).get();

    /*LISTING STYLE*/
    let mlsStyle = $("#x_wrapperTable table table tbody > tr:nth-child(6) > td:nth-child(2) span").map(function(){
        return $(this).text();
    }).get();
    ...
    /*LISTING STYLE*/
    let mlsAcres = $("#x_wrapperTable table table tbody > tr:nth-child(8) > td:nth-child(2) span").map(function(){
        return $(this).text();
    }).get();
