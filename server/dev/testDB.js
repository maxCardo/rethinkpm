...
const loadTestDB = async () => {
    MongoClient.connect(dataBase, { useNewUrlParser: true }, async (err, client) => {
        if (err) {
            return console.log('Error: problem connecting to mongoDB getVendor');
        }
        const db = client.db();
        const data = await db.collection('chats').find({}).sort({ last_active: -1 }).limit(100).toArray()
        ...
        data.map( async (record) => {
            ...
            if (chats.length > 1) {
                let chat = await new ChatInq({ inq: inq._id, messages: chats, unread: false }) 
                chat.save()
            }
            pros.save()
            inq.save()
