...
router.post('/', async (req, res) => {
    try {
        ...
        let pros;
        phoneNumber
            ? (pros = await RentLeadPros.findOne({
                  'phone.phoneNumber': phoneNumber,
              }))
            : (pros = await RentLeadPros.findOne({email: email}));
...
router.get('/open_leads', async (req, res) => {
    ...
    try {
        const leads = await RentLeadInq.find({
...
router.get('/chats', async (req, res) => {
    ...
    try {
        const chats = await ChatInq.find().populate({
            path: 'inq',
            select: 'prospect',
            select: 'listing',
...
router.get('/chat/:inq_id', async (req, res) => {
    ...
    try {
        let chat = await ChatInq.findOne({inq: req.params.inq_id}).populate({
            path: 'inq',
            select: 'prospect',
            select: 'listing',
            populate: {path: 'prospect', select: 'name'},
        ...
        if (!chat) {
            ...
            chat.save(async (err, item) => {
                chat = await ChatInq.findOne(item).populate({
                    path: 'inq',
                    select: 'prospect',
                    select: 'listing',
                    populate: {path: 'prospect', select: 'name'},
...
router.post('/chat', async (req, res) => {
    try {
        ...
        if (!chat) {
            chat = await new ChatInq({inq: inq._id});
        }
