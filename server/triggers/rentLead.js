...
cron.schedule(' 34 12,18 * * 0-5', async () => {
  ...
  const newLeads = await inq.find({'status.currentStatus': 'new'}).populate('prospect')
  //loop through and send email or text
  newLeads.map(async lead => {
    let {prospect:{phone:{phoneType}},notes} = lead;
    phoneType === 'mobile' ? (console.log('call: ')) : (console.log('email:'));
    notes.unshift({ note:`Auto follow up via ${phoneType ==='mobile' ? ('sms'):('email')}`});
    //update and save record
    lead.status.new.numFlwUps = await lead.status.new.numFlwUps +1;
    let {status:{ new:{ numFlwUps }}} = lead

    if (phoneType === 'mobile' && numFlwUps >= 4 || phoneType != 'mobile' && numFlwUps >= 1) {
